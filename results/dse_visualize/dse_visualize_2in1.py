...
Runs the SM × L2 grid search via run_dse() and plots a single heatmap:
  - Cell color  → Power (W)   – YlOrRd colorbar (warm red/orange)
  - Upper label → Power value (W)
  - Lower label → Latency value (ms) in parentheses
...
----------------
  ...
  python dse_visualize_2in1.py --area 200 --base_hw Thor \
      ...
      --mem_freq 10667 --mem_bitwidth 144 \
...
def _annotate_cells_combined(ax, pwr_masked, lat_masked, pwr_vmin, pwr_vmax):
    """Write two lines in every non-masked cell.

    Upper line: power value (W), e.g. "18.7W"
    Lower line: latency value in parentheses (ms), e.g. "(42.3ms)"
    Text color adapts to power cell brightness.
    """
    nrows, ncols = pwr_masked.shape
    for i in range(nrows):
        for j in range(ncols):
            pwr_val = pwr_masked[i, j]
            lat_val = lat_masked[i, j]
            if np.ma.is_masked(pwr_val) or np.isnan(pwr_val):
                continue
            normalized = (
                (float(pwr_val) - pwr_vmin) / (pwr_vmax - pwr_vmin)
                if pwr_vmax > pwr_vmin
                else 0.5
            )
...
def plot_combined_heatmap(
    ...
    *,
    ...
    """Draw a single 2D heatmap colored by power, annotated with power + latency.

    Each cell shows:
      upper line → power value (W)
      lower line → latency value in parentheses (ms)
    ...
    cell_w = 0.3  # inches per SM column
    ...
    cmap_obj = _make_gray_cmap("YlOrRd")
    ax.imshow(
        pwr_masked,
        cmap=cmap_obj,
        vmin=pwr_vmin,
    ...
    _annotate_cells_combined(ax, pwr_masked, lat_masked, pwr_vmin, pwr_vmax)
    ...
    ax.set_yticklabels([f"{v:.1f}" for v in l2_list], fontsize=7)
...
def main():
    ...
    # ── Colorscale bounds (fixed) ────────────────────────────────────────────
    pwr_vmin, pwr_vmax = 20.0, 100.0

    # Short tag for filenames
    tag = (
        f"{args.base_hw}_{args.inference_config}_{llm_model}_"
        f"{args.precision}_{args.area}mm2_{args.latency_limit:.0f}ms"
    ).replace("/", "_")

    print("\n  Generating combined heatmap …")

    # ── Combined heatmap: colored by power, annotated with power + latency ───
