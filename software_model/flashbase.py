...
from software_model.utils import DataType, Tensor, DeviceType, L2AccessType, L2Cache, data_type_dict
...
class FlashAttention(Operator):
    ...
    * Global I/O traffic driven by repeatedly streaming K/V blocks.
    ...
    class ComputationGraph:
        ...
        """
        def __init__(self, seq_len_q: int, seq_len_kv: int, head_dim: int, batch_size: int, num_heads_q: int, num_heads_kv: int, num_splits: int, qkv_data_type: DataType, output_data_type: DataType, intermediate_data_type: DataType):
        ...
        def display(self):
            print(f"ComputationGraph(seq_len_q={self.seq_len_q}, seq_len_kv={self.seq_len_kv}, head_dim={self.head_dim}, batch_size={self.batch_size}, num_heads_q={self.num_heads_q}, num_heads_kv={self.num_heads_kv}, num_splits={self.num_splits}, qkv_data_type={self.qkv_data_type.name}, output_data_type={self.output_data_type.name}, intermediate_data_type={self.intermediate_data_type.name})")
    ...
    class L2CacheFlashAttention(L2Cache):
            ...
            if L2Cache_previous:
                ...
                while L2Cache_previous.resident_tiles:
                    ...
                    if tile.access_type == L2AccessType.OUTPUT:
                        self.resident_tiles[L2Cache.Tile(L2AccessType.OUTPUT, tile.coord_tuple)] = None
        ...
        def access(
            ...
            """
            height = self.seq_len_q_tiles if access_type == L2AccessType.Q or access_type == L2AccessType.OUTPUT else \
                self.seq_len_kv_tiles if access_type == L2AccessType.K or access_type == L2AccessType.V else \
                None
            width = self.head_dim_tiles if access_type == L2AccessType.Q or access_type == L2AccessType.OUTPUT else \
                self.head_dim_tiles if access_type == L2AccessType.K or access_type == L2AccessType.V else \
                None
            num_heads = self.num_heads_q if access_type == L2AccessType.Q or access_type == L2AccessType.OUTPUT else self.num_heads_kv
            tile_size = self.qkv_tile_size if access_type in [L2AccessType.Q, L2AccessType.K, L2AccessType.V] else self.output_tile_size
            ...
            for i in range(coord_tuple[0], coord_tuple[0] + scope_tuple[0], L2Cache.TILE_LENGTH):
                for j in range(coord_tuple[1], coord_tuple[1] + scope_tuple[1], L2Cache.TILE_LENGTH):
                    ...
                    if tile in self.resident_tiles: # HIT
                        ...
                        self.resident_tiles[self.Tile3D(access_type, (i, j, coord_tuple[2]))] = None
        ...
        def evict_oldest_tile(self):
            ...
            tile_size = self.qkv_tile_size if oldest_tile.access_type in (L2AccessType.Q, L2AccessType.K, L2AccessType.V) else self.output_tile_size if oldest_tile.access_type == L2AccessType.OUTPUT else None
    ...
    def compile_and_simulate(
        ...
        if compile_mode == "exhaustive":
            ...
            for l2_tile_seq_len_q_log2 in range(min(5, floor(log2(self.computational_graph.seq_len_q))), ceil(log2(self.computational_graph.seq_len_q)) + 1):
                l2_tile_seq_len_q = 2 ** l2_tile_seq_len_q_log2
                
                # Determine L2 tile size for KV based on splits
                l2_tile_seq_len_kv = ceil(self.computational_graph.seq_len_kv / self.num_splits)
                l2_tile_seq_len_kv_log2 = floor(log2(l2_tile_seq_len_kv))
                
                # Iterate over possible L1 tile sizes for Q
                for l1_tile_seq_len_q_log2 in range(min(5, l2_tile_seq_len_q_log2), l2_tile_seq_len_q_log2 + 1):
                    ...
                    for l1_tile_seq_len_kv_log2 in range(min(5, l2_tile_seq_len_kv_log2), l2_tile_seq_len_kv_log2 + 1):
                        ...
                        working_set_size_bytes = (
                            l1_tile_seq_len_q * self.computational_graph.head_dim * max(self.pcb_module.compute_module.core_count / ceil(l2_tile_seq_len_q / l1_tile_seq_len_q), 1) * 2 * self.qkv_data_type.word_size
                            + l1_tile_seq_len_kv * self.computational_graph.head_dim * min(self.pcb_module.compute_module.core_count, ceil(l2_tile_seq_len_kv / l1_tile_seq_len_kv)) * 2 * self.output_data_type.word_size
                        )
                        ...
                        if (
                            working_set_size_bytes > self.pcb_module.compute_module.l2_size or working_set_size_bytes < self.pcb_module.compute_module.core.SRAM_size
                        ):
                            continue
                        
                        # Constraint: Working set must fit in SRAM (L1)
                        if (3 * l1_tile_seq_len_q * self.computational_graph.head_dim * self.qkv_data_type.word_size + 4 * l1_tile_seq_len_kv * self.computational_graph.head_dim * self.output_data_type.word_size + l1_tile_seq_len_q * l1_tile_seq_len_kv) > self.pcb_module.compute_module.core.SRAM_size:
                            continue
    ...
    def simulate(
        ...
        l2_tiles = np.empty(
            [batch_size, num_heads_q, ceil(seq_len_q / l2_tile_seq_q), ceil(seq_len_kv / l2_tile_seq_kv)],
        ...
        for (b, h, l2_tile_i, l2_tile_j) in self.generate_tile_loops_with_BH(
            ...
            if (l2_tile_i + 1) * l2_tile_seq_q + (seq_len_kv - seq_len_q) >= l2_tile_j * l2_tile_seq_kv or not self.is_causal:
            ...
            for i in range(len(active_l2_tile_list)):
                ...
                read_q_tile, read_kv_tile, write_output_tile = temp_l2_tile.simulate_l2_tile_io_tile()
                read_q_head = temp_l2_batch * num_heads_q + temp_l2_head
                read_kv_head = temp_l2_batch * num_heads_kv + temp_l2_head // (num_heads_q // num_heads_kv)
                ...
                current_write_output_bytes += l2cache.access(
                    L2AccessType.OUTPUT,
                    (write_output_tile[0][0], write_output_tile[0][1], write_output_head),
                    (write_output_tile[1][0], write_output_tile[1][1]),
                )

            current_read_bytes = (
                current_read_q_bytes
                + current_read_kv_bytes
            )
            
            # Calculate cycles for memory operations
            current_read_cycle_count = ceil(
                current_read_bytes / (pcb_module.io_module.bandwidth * pcb_module.io_module.bandwidth_efficiency / pcb_module.compute_module.clock_freq)
            ) + pcb_module.io_module.latency_cycles
            
            previous_write_cycle_count = ceil(
                previous_write_output_bytes / (pcb_module.io_module.bandwidth * pcb_module.io_module.bandwidth_efficiency / pcb_module.compute_module.clock_freq)
            )
        ...
        if self.num_splits > 1:
            reduction_counts = seq_len_q * head_dim * self.num_splits * batch_size * num_heads_q / pcb_module.compute_module.core_count
            
            reduction_io_read_counts = seq_len_q * head_dim * self.num_splits * batch_size * num_heads_q * qkv_data_type.word_size
            
            reduction_io_read_cycle_counts = ceil(reduction_io_read_counts / (pcb_module.io_module.bandwidth * pcb_module.io_module.bandwidth_efficiency / pcb_module.compute_module.clock_freq))
            
            reduction_io_write_counts = seq_len_q * head_dim * batch_size * num_heads_q * qkv_data_type.word_size
            
            reduction_io_write_cycle_counts = ceil(reduction_io_write_counts / (pcb_module.io_module.bandwidth * pcb_module.io_module.bandwidth_efficiency / pcb_module.compute_module.clock_freq))
            
            # Compute cycles for reduction (FMA)
            reduction_compute_cycles = reduction_counts / (pcb_module.compute_module.core.vector_unit.get_throughput_per_cycle(qkv_data_type, "fma") * pcb_module.compute_module.core.sublane_count)
            
            # Addressing overhead cycles
            addressing_compute_cycles = 12 * reduction_counts / (pcb_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type_dict["int32"], "fma") * pcb_module.compute_module.core.sublane_count)
            
            total_cycle_count += reduction_io_read_cycle_counts + reduction_io_write_cycle_counts + reduction_compute_cycles + addressing_compute_cycles
            
        # Add latency for data type conversion if necessary
        if output_data_type != qkv_data_type:
            # Data Type Conversion IO
            data_type_conversion_io_counts = seq_len_q * head_dim * batch_size * num_heads_q * (qkv_data_type.word_size + output_data_type.word_size)
            
            data_type_conversion_io_cycle_counts = ceil(data_type_conversion_io_counts / (pcb_module.io_module.bandwidth * pcb_module.io_module.bandwidth_efficiency / pcb_module.compute_module.clock_freq))
    ...
    class L2TileSimulator:
        ...
        def simulate_l2_tile_io_tile(
            self,
        ) -> tuple[tuple[tuple[int, int], tuple[int, int]], tuple[tuple[int, int], tuple[int, int]], tuple[tuple[int, int], tuple[int, int]]]:
            """
        ...
        def simulate_l2_tile_io_cycle_count(
            ...
            """
            return ceil(
                seq_len * head_dim * data_type.word_size / (chiplet_module.io_module.bandwidth * chiplet_module.io_module.bandwidth_efficiency / chiplet_module.compute_module.clock_freq)
            )
        ...
        def simulate_l2_tile_compute_cycle_count(
            ...
            for l1_tile_i in range(ceil(seq_len_q / l1_tile_seq_q)):
                for l1_tile_j in range(ceil(seq_len_kv / l1_tile_seq_kv)):
                    ...
                    current_read_cycle_count += chiplet_module.compute_module.l2_latency_cycles
                    
                    l1_tile = l1_tiles[l1_tile_i, l1_tile_j]
                    
                    # Apply causal mask filtering
                    if self.position_len_q + (l1_tile_i + 1) * l1_tile.seq_len_q + self.position_offset >= self.position_len_kv + l1_tile_j * l1_tile.seq_len_kv or not self.is_causal:
                        pass
                    else:
                        continue
                    ...
                    if self.position_len_q + l1_tile_i * l1_tile.seq_len_q + self.position_offset < self.position_len_kv + (l1_tile_j + 1) * l1_tile.seq_len_kv and self.is_causal:
            ...
            quant_output_cycle_count = ceil(
                seq_len_q * head_dim / (chiplet_module.compute_module.core.sublane_count * chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="cvt"))
            )
            
            # Cycles for writing back output to L2
            write_back_cycle_count = ceil(
                seq_len_q * head_dim * output_data_type.word_size / (chiplet_module.compute_module.l2_bandwidth_per_cycle)
            )
            
            total_cycle_count += previous_compute_cycle_count + quant_output_cycle_count + write_back_cycle_count
            
            # Store tile coordinates for cache simulation
            self.read_q_tile = ((self.position_len_q, 0), (total_l1_tile_q, head_dim))
            self.read_kv_tile = ((self.position_len_kv, 0), (total_l1_tile_kv, head_dim))
    ...
    class L1TileSimulator:
        ...
        def simuate_l1_tile_mask_cycle_count(
            ...
            """
            return ceil(seq_len_q * seq_len_kv / (chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="fma") * chiplet_module.compute_module.core.sublane_count))
        ...
        def simulate_l1_tile_compute_cycle_count(
            ...
            assert((3 * seq_len_q * head_dim + 4 * seq_len_kv * head_dim + seq_len_q * seq_len_kv) <= chiplet_module.compute_module.core.SRAM_size // qkv_data_type.word_size)
            ...
            compute_mat1_cycle_count = ceil(
                ...
                * seq_len_q
                * seq_len_kv
                / (chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="reduction") * chiplet_module.compute_module.core.sublane_count)
            )

            # 2. Softmax Logic
            # Reduction (max)
            reduce_mat1_cycle_count = seq_len_q * seq_len_kv / (chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="reduction") * chiplet_module.compute_module.core.sublane_count)
            
            # Exp
            pointwise_mat1_cycle_count = seq_len_q * seq_len_kv / (chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="exp2") * chiplet_module.compute_module.core.sublane_count)
            
            # Sum
            pointwise_l_cycle_count = seq_len_q / (chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="fma") * chiplet_module.compute_module.core.sublane_count)
            
            # Quantization/Conversion
            quant_p_cycle_count = seq_len_q * seq_len_kv / (chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="cvt") * chiplet_module.compute_module.core.sublane_count)
            ...
            # 4. Output Update (FMA)
            update_output_cycle_count = seq_len_q * head_dim / (chiplet_module.compute_module.core.vector_unit.get_throughput_per_cycle(data_type=intermediate_data_type, operation="fma") * chiplet_module.compute_module.core.sublane_count)
