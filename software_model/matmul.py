...
class L2CacheMatmul(L2Cache):
    def __init__(
        ...
        self.k_scale_tiles = (
            ceil(K / L2Cache.TILE_LENGTH / scale_block_size)
            if scale_block_size
            else None
        )
        ...
        if L2Cache_previous:
            ...
            while L2Cache_previous.resident_tiles:
                ...
                if tile.access_type == L2AccessType.OUTPUT_SCALE:
                    self.resident_tiles[
                        L2Cache.Tile(L2AccessType.ACTIVATION_SCALE, tile.coord_tuple)
                    ] = None
    ...
    def evict_oldest_tile(self):
        ...
        tile_size = (
            ...
            if oldest_tile.access_type == L2AccessType.WEIGHT
            else self.output_tile_size
...
class Matmul(Operator):
    ...
    class Mapping:
        ...
        def display(self):
            ...
            print(
                f"cta_m: {self.cta_m}, cta_n: {self.cta_n}, cta_k: {self.cta_k}, stages: {self.stages}"
            )
            print(
                f"swizzle_size: {self.swizzle_size}, raster_order: {'ALONG_M' if self.raster_order == Matmul.RasterOrder.ALONG_M else 'ALONG_N'}"
            )
            print(
                f"is_2sm_mode: {self.is_2sm_mode}, active_ctas_per_core: {self.active_ctas_per_core}"
            )
    ...
    def __init__(
        ...
        assert (
            self.device_type == DeviceType.ORIN
            and weight_dtype.name in ("fp16", "int8", "int4")
        ) or (
            self.device_type == DeviceType.THOR and weight_dtype.name in ("fp8", "fp4")
    ...
    def compile_and_simulate(
        ...
        N = self.N
        K = self.K
        assert (self.device_type == DeviceType.THOR and M >= 64) or (
            self.device_type == DeviceType.ORIN and M >= 32
            ...
            for cta_n in cta_n_list:
                for cta_k in cta_k_list:
                    for swizzle_size in [1, 2, 4, 8]:
                        ...
                        if self.device_type == DeviceType.ORIN:
                            if (
                                self.activation_dtype.name == "fp16"
                            ):  # suppose HMMA.m16n8k16
                                register_usage = (
                                    cta_m
                                    * cta_n
                                ...
                                self.activation_dtype.name == "int8"
                            ):  # suppose HMMA.m16n8k32
                                register_usage = (
                                    cta_m
                                    * cta_n
                                    * self.intermediate_dtype.word_size
                        ...
                        raster_order = (
                            Matmul.RasterOrder.ALONG_M
                            if M <= N
                            else Matmul.RasterOrder.ALONG_N
                        )
    ...
    def simulate(
            ...
            for k_start in range(0, K, cta_k):
                ...
                curr_wave.append(
                    self.L2TileSimulator(
                        max(m_coords) - min(m_coords) + cta_m,
                        max(n_coords) - min(n_coords) + cta_n,
                        k_size,
                        min(m_coords),
                        min(n_coords),
                        k_start,
        ...
        for l2_tiles in waves:
            ...
            self.l2_access_size += (
                sum(
                    l2_tile.l1_input_io_cycle_count + l2_tile.l1_output_io_cycle_count
    ...
    class L2TileSimulator:
        def __init__(
            ...
            self.l1_output_io_cycle_count = self.l1_tile.output_io_cycle_count * len(
                self.cta_chunk
            )
            self.l1_compute_cycle_count = self.l1_tile.compute_cycle_count
            self.compute_cycle_count = (
                self.l1_compute_cycle_count * active_ctas_per_core
            )
        ...
        def get_input_io_cycle_count(self):
            ...
            if self.weight_dtype.name == "fp4":
                ...
                M_K_io_cycle_count += self.simulate_l2_tile_io_cycle_count(
                    ...
                    (
                        self.coord_M,
                        (self.coord_K // scale_block_size)
                        // L2Cache.TILE_LENGTH
                        * L2Cache.TILE_LENGTH,
                    ),
                    (
                        self.M,
                        ceil((self.K // scale_block_size) / L2Cache.TILE_LENGTH)
                        * L2Cache.TILE_LENGTH,
                    ),
                    self.pcb_module,
                )
                ...
                if self.output_dtype.name == "fp4":
                    ...
                    M_N_io_cycle_count += self.simulate_l2_tile_io_cycle_count(
                        ...
                        (
                            self.l1_tile.M,
                            ceil(
                                (self.l1_tile.N // scale_block_size)
                                / L2Cache.TILE_LENGTH
                            )
                            * L2Cache.TILE_LENGTH,
                        ),
    ...
    class L1TileSimulator:
        def __init__(
            ...
            M_K_io_cycle_count = self.simulate_l1_tile_io_cycle_count(
                self.M // 2 if self.is_2sm_mode else self.M,
                self.K,
                self.activation_dtype,
                self.pcb_module,
            )
            K_N_io_cycle_count = self.simulate_l1_tile_io_cycle_count(
                self.K,
                self.N // 2 if self.is_2sm_mode else self.N,
                self.weight_dtype,
                self.pcb_module,
            )
