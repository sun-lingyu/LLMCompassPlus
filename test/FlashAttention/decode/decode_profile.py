...
dtype_map = {
    ...
    "int8": torch.int8,
    "int4": torch.int8,
}  # int4 mapped to int8 as PyTorch does not have int4 dtype
...
# 解码时 Query 长度通常为 1
q = torch.randn(
    batch_size, seqlen_q, nheads_q, headdim, device=device, dtype=activation_dtype
)
...
k_cache = torch.randn(
    batch_size, max_seqlen_kv, nheads_kv, headdim, device=device, dtype=weight_dtype
)
v_cache = torch.randn(
    batch_size, max_seqlen_kv, nheads_kv, headdim, device=device, dtype=weight_dtype
)
...
k_new = torch.randn(
    batch_size, seqlen_kv_new, nheads_kv, headdim, device=device, dtype=weight_dtype
)
v_new = torch.randn(
    batch_size, seqlen_kv_new, nheads_kv, headdim, device=device, dtype=weight_dtype
)
