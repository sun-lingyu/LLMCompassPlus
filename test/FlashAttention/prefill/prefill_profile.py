...
dtype_map = {'fp16': torch.float16, 'bf16': torch.bfloat16, 'int32': torch.int32, 'fp32': torch.float32, 'int8': torch.int8, 'int4': torch.int8}
...
Q = torch.randn(B, S, Q_heads, H_dim, device=device, dtype=activation_data_type).contiguous()
K = torch.randn(B, S, KV_heads, H_dim, device=device, dtype=weight_data_type).contiguous()
V = torch.randn(B, S, KV_heads, H_dim, device=device, dtype=weight_data_type).contiguous()
