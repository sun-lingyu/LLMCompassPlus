...
def test_and_save_latency_prefill(
    ...
    latency_list = []
    for (idx, (batch_size, num_heads_q, num_heads_kv, head_dim, seq_len)) in enumerate(test_params):
        print(f"problem: batch_size {batch_size} heads_q {num_heads_q} heads_kv {num_heads_kv} head_dim {head_dim} seq_len {seq_len}")
        ...
        print(f"Ours: {latency:.3f} ms, Roofline: {roofline_latency:.3f} ms, GPU: {gpu_latency:.3f} ms")
        latency_list.append((batch_size, num_heads_q, num_heads_kv, head_dim, seq_len, latency, roofline_latency, gpu_latency))

    # Save results
    df_out = pd.DataFrame(
        latency_list,
        columns=["batch_size", "num_heads_q", "num_heads_kv", "head_dim", "seq_len", "Ours", "Roofline", "GPU"]
    )
...
def test_and_save_latency_decode(
    ...
    latency_list = []
    for (idx, (batch_size, num_heads_q, num_heads_kv, head_dim, seq_len_q, seq_len_kv)) in enumerate(test_params):
        print(f"problem: batch_size {batch_size} heads_q {num_heads_q} heads_kv {num_heads_kv} head_dim {head_dim} seq_len_q {seq_len_q} seq_len_kv {seq_len_kv}")
        ...
        print(f"Ours: {latency:.3f} ms, Roofline: {roofline_latency:.3f} ms, GPU: {gpu_latency:.3f} ms")
        latency_list.append((batch_size, num_heads_q, num_heads_kv, head_dim, seq_len_q, seq_len_kv, latency, roofline_latency, gpu_latency))

    # Save results
    df_out = pd.DataFrame(
        latency_list,
        columns=["batch_size", "num_heads_q", "num_heads_kv", "head_dim", "seq_len_q", "seq_len_kv", "Ours", "Roofline", "GPU"]
    )
    df_out.to_csv(file_name, index=False)
        ...
        for batch in M:
          for slq in seq_len_q:
            ...
            for slk in range(slk_begin, slk_end + 1, 128):
              test_params.append((batch, num_heads_q, num_heads_kv, head_dim, slq, slk))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, choices=["InternVision", "Qwen3_0_6B", "Qwen3_1_7B", "Qwen3_4B", "Qwen3_8B", "all"], default="all")
    parser.add_argument("--precision", type=str, choices=["fp32", "fp16", "int8", "int4", "all"], default="all")
    parser.add_argument("--update", nargs="+", choices=["ours", "roofline", "gpu", "all"], default=["all"])
    ...
    if args.model != "all":
      ...
      model_list = ["InternVision", "Qwen3_0_6B", "Qwen3_1_7B", "Qwen3_4B", "Qwen3_8B"]
