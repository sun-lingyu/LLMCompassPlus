...
def fit_and_analyze_rails(X_raw, y_soc, y_mem, args):
    # 1. 定义完整的特征全集 (顺序必须与 X_raw 的列顺序一致)
    ...
    # 2.在此处配置你想使用的特征子集
    # ==============================================================================
    # SoC 轨：通常只选 FMA 和 DRAM，防止 Reg Access 吃掉系数
    # 你可以在列表里随意添加/删除，例如 ["SysArr FMA", "Reg Access", "DRAM Access"]
    ...
    def fit_single_rail(X_full, y, features_to_use, rail_label, enforce_positive=True, fit_intercept=True):
        ...
        # 3. 标准化 (with_mean=False)
        scaler = StandardScaler(with_mean=False)
        X_scaled = scaler.fit_transform(X_subset)

        # 4. 拟合
        # 注意：Mem轨之前发现可能是负斜率，如果你想允许负数，就把 enforce_positive 设为 False
    ...
    # 拟合 SoC (通常强制为正 positive=True)
    res_soc = fit_single_rail(X_raw, y_soc, soc_features_to_use, 
                              "Rail 1: VDD_GPU_SOC", enforce_positive=True)
    
    # 拟合 Mem (之前你的代码里去掉了positive=True，这里我也保留这个选项)
    # 如果你想强制非负，把下面的 False 改为 True
    ...
    for i, name in enumerate(full_feature_names):
        val = res_soc['coefs'][i]
        # 标记是否是被强制为0的（即不在子集里的），或者是拟合出来本身就是0
    ...
    for i, name in enumerate(full_feature_names):
        ...
        # 为了简洁，Mem 轨如果某项是 Ignore 且值为0，可以选择不打印，或者打印全部
        # 这里打印全部以保持格式统一
        print(f"{name:<15} | {val:.6e}           | {status}")
    ...
    # 绘图
    # ---------------------------------------------------------
    # 注意：你需要确保 plot_fitting_results 函数接受 title_suffix 参数
    # 如果原函数没有，请删除 title_suffix 参数
