...
def get_model_shape(model):
    ...
    N_shapes = {
        "qkv_proj": model_shapes["head_dim"]
        * (
            model_shapes["num_key_value_heads"] * 2
            + model_shapes["num_attention_heads"]
        ),
        "o_proj": model_shapes["hidden_size"],
        "up_proj": model_shapes["intermediate_size"] * 2
        if model_shapes["hidden_act"] == "silu"
        else model_shapes["intermediate_size"],  # SiLU/GELU
...
def get_output_dtype(activation_dtype: DataType, op_name: str, is_test: bool):
    assert op_name in ("qkv_proj", "o_proj", "up_proj", "down_proj"), (
        "op_name must be one of (qkv_proj, o_proj, up_proj, down_proj)"
    )
    ...
    if activation_dtype.name == "int8":
        if is_test or op_name == "up_proj":
            return data_type_dict[
                "int8"
            ]  # CUTLASS profiler does not support S8 GEMM with F16 output
        else:
    ...
    if activation_dtype.name == "fp4":
        if op_name == "qkv_proj":
            ...
            if is_test:
                return data_type_dict[
                    "fp8"
                ]  # CUTLASS profiler does not support F4 GEMM with F16 output
            return data_type_dict["fp16"]
