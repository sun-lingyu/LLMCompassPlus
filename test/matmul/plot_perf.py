...
if args.precision in ("int8", "fp16"):
    axes.text(0.5, 0.01, f"MAPE: Ours {mape_ours:.1f}%, LLMCompass {mape_baseline:.1f}%, Roofline {mape_roofline:.1f}%", ha='center', va='bottom', transform=axes.transAxes, fontsize=6)
else:
    axes.text(0.5, 0.01, f"MAPE: Ours {mape_ours:.1f}%, Roofline {mape_roofline:.1f}%", ha='center', va='bottom', transform=axes.transAxes, fontsize=6)
...
for idx, op_name in enumerate(["qkv_proj", "o_proj", "up_proj", "down_proj"]):
    if args.mode == "prefill":
        latency_table = pd.read_csv(f"{file_dir}/results_perf/{args.model}/{args.precision}/{args.mode}/{op_name}_CP.csv")
        plot_latency(latency_table, axes_1[idx], title=op_name, precision=args.precision, is_first=(idx==0), is_last=(idx==3))
    
    latency_table = pd.read_csv(f"{file_dir}/results_perf/{args.model}/{args.precision}/{args.mode}/{op_name}_TP.csv")
    plot_latency(latency_table, axes[idx], title=op_name, precision=args.precision, is_first=(idx==0), is_last=(idx==3))
