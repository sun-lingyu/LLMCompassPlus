...
def cutlass_gemm_min_latency_remote(
    ...
    if precision == "fp16":
        ...
        kernel_prefix=f"cutlass3x_sm100_tensorop_gemm_f8_f8_f32_void_"
    ...
    cutlass_cmd += ["--llc-capacity=524288"] # llc-capacity set to very large 512MB to avoid any L2 residency
...
def test_and_save_latency(
    ...
    for (idx, (M, N, K)) in enumerate(test_problems):
        ...
        model = Matmul(activation_dtype, weight_dtype, intermediate_dtype, output_dtype, device)
        _ = model(
        ...
        if update_ours_only:
            ...
            baseline_latency =  get_baseline_latency(M, N, K, args.precision) if precision in ("int8", "fp16") else -1
            roofline_latency = 1000 * model.roofline_model(pcb)
            port = 9129 if args.device == "Orin" else 9147
            cutlass_latency = cutlass_gemm_min_latency_remote(M, N, K, precision, output_dtype, port)[0] if precision != "int4" else marlin_gemm_latency_remote(M, N, K)
...
if __name__ == "__main__":
    ...
    for op_name in ["qkv_proj", "o_proj", "up_proj", "down_proj"]:
        ...
        if args.mode == "prefill":
            ...
            test_and_save_latency(test_problems, f"{file_dir}/results_perf/{args.model}/{args.precision}/{args.mode}/{op_name}_CP.csv", args.precision, op_name, args.update_ours_only, args.device)
        ...
        test_and_save_latency(test_problems, f"{file_dir}/results_perf/{args.model}/{args.precision}/{args.mode}/{op_name}_TP.csv", args.precision, op_name, args.update_ours_only, args.device)
