...
def test_latency(
    ...
    baseline_latency =  get_baseline_latency(M, N, K)
    roofline_latency = 1000 * (model.roofline_model(pcb) + 2773 / pcb.compute_module.clock_freq)
    cutlass_latency = cutlass_gemm_min_latency_remote(M, N, K)
    ...
    return latency, baseline_latency, roofline_latency, cutlass_latency
...
def test_and_save_latency(
    ...
    latency_list = []
    for shape in test_problems:
        latency_list.append(test_latency(*shape))
    df = pd.DataFrame(
        latency_list,
...
if __name__ == "__main__":
    ...
    K_shapes = {
        ...
        "o_proj": model_shapes["head_dim"] * model_shapes["num_key_value_heads"],
    ...
    N_shapes = {
        ...
        "up_proj": model_shapes["intermediate_size"],
        "down_proj": model_shapes["hidden_size"]
        }
    M = 1024 if args.mode == "prefill" else 64
    assert(M % 4 == 0)

    os.makedirs(f"{file_dir}/{args.model}/{args.mode}", exist_ok=True)
    for test in ["qkv_proj", "o_proj", "up_proj", "down_proj"]:
        ...
        if args.mode == "prefill":
            ...
            test_and_save_latency(test_problems, f"{file_dir}/{args.model}/{args.mode}/{test}_CP.csv")
        ...
        test_and_save_latency(test_problems, f"{file_dir}/{args.model}/{args.mode}/{test}_TP.csv")
