...
latency_table = pd.read_csv(f"{file_dir}/results_perf/{args.mode}.csv")
plot_latency(latency_table, axes, title=args.mode, is_first=True, is_last=True)

mape_ours = np.mean(np.abs((latency_table['Ours'] - latency_table['Measured']) / latency_table['Measured'])) * 100
mape_roofline = np.mean(np.abs((latency_table['Roofline'] - latency_table['Measured']) / latency_table['Measured'])) * 100
...
fig.savefig(f"{file_dir}/results_perf/{args.mode}.png", dpi=300)
